...
import {
  ...
  TrpcResource,
  TrpcResourceOptions,
} from './trpcResource.types';

function trpcResource<TDef extends ResolverDef>(
  procedure: Resolver<TDef>,
  input: () => TDef['input'] | undefined,
  options?: TrpcResourceOptions<TDef['output']>
): TrpcResource<TDef> {
  const currentInput = computed(input);

  const value = linkedSignal<TDef['output'] | undefined>(
    options?.valueComputation ??
  ...
  const refresh = async (
    ...
    try {
      ...
      if (isTRPCClientError(err)) {
        ...
        console.error(
          'A non-tRPC error has occured on this trpcResource: ',
          String(err)
        );
      }
...
function debugTrpcResource<TDef extends ResolverDef>(
  _trpcResource: TrpcResource<TDef>
) {
  return {
