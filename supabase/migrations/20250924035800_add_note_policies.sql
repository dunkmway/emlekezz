...
CREATE INDEX "user_id" ON "public"."note" USING "btree" ("user_id");



ALTER TABLE ONLY "public"."note"
    ADD CONSTRAINT "note_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON UPDATE CASCADE ON DELETE CASCADE;



CREATE POLICY "Enable users to view their own data only" ON "public"."note" TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));
